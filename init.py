...
class TGView:
    ...
    def messages(self):
        messages = []
        post_divs = self.posts_soup.find_all('div', class_='tgme_widget_message_wrap')
        print(f"Found {len(post_divs)} posts")  # Debug
        for post_div in post_divs:
            # Extract message content
            content_div = post_div.find('div', class_='tgme_widget_message_text')
            content = content_div.text if content_div else None

            # Extract date
            time_elem = post_div.find('time', class_='datetime')
            date = time_elem.text if time_elem else None

            # Extract views
            views_span = post_div.find('span', class_='tgme_widget_message_views')
            views = views_span.text if views_span else None

            # Extract images
            images = []
            photo_wraps = post_div.find_all('a', class_='tgme_widget_message_photo_wrap')
            for photo in photo_wraps:
                if 'style' in photo.attrs:
                    url = re.search(r"background-image:url\('(.*?)'\)", photo['style'])
                    if url:
                        images.append(url.group(1))
